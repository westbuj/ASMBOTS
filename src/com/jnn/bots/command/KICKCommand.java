...
public class KICKCommand implements COMMANDS {
	...
	public boolean operate(Bot b,ArrayList<String> params) {
		...
		if (b.isReversed)
		{
			x= CMath.revX(b.regs.get("POSX").mValue);
			y = CMath.revY(b.regs.get("POSY").mValue);
		}
		else
		{
			x= b.regs.get("POSX").mValue;
			y = b.regs.get("POSY").mValue;
		}
		...
		//	fsg
			...
			if (b.isReversed)
				rawDirection =  CMath.revA(b.regs.get(params.get(0)).mValue);
			else
				rawDirection =  b.regs.get(params.get(0)).mValue;
			
			int accuracy = rand.nextInt(100);
			//75 percent of the time, they will be very accurate
			if (accuracy > 25)
			{
